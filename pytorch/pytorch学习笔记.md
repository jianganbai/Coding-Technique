...
- `torch.utils.data.IterableDataset`：Iterable-style，按迭代器方式输出样本，可实现dynamic batch
  ...
  - 必须实现`__init__`和`__iter__`，可选实现`__len__`
  
  - 不需要一次性读入数据集，也不需要知道数据集大小
  
  - 自行决定读取样本的顺序，无法使用sampler或者batch_sampler，无法使用shuffle
  
  - num_workers > 0：需要在`__iter__`内特殊处理，否则每个worker都会把数据全读一遍
    
    - ```python
      worker_info = torch.utils.data.get_worker_info()
      if worker_info is None:  # num_workers = 0
          id_iter = range(num_sample)
      else:  # num_workers > 0
          id_iter = range(worker_info.id, num_sample, worker_info.num_workers)  # 手动切分每个worker的遍历index
      ```
    
    - 对于DDP，需要根据`global_worker_id`进行划分
...
- `torch.utils.data.ConcatDataset([Dataset1, Dataset2])`：将多个数据集拼接起来
  ...
  - 对于`IterableDataset`，需要使用`torch.utils.data.ChainDataset`
...
- `num_workers`：开多少个worker进程加载batch
  - =0则主进程用于加载，>0则主进程不加载
  - 主进程使用batchsampler将batch分配给worker
  - worker以batch为单位读入RAM，至多预先读取`num_workers`个batch到RAM
  - 主进程在RAM中找读入的batch，若未找到则等worker加载完成
  - `worker_info = torch.utils.data.get_worker_info()`：在dataset得知是否有多个worker
    - `worker_info.id`：从0开始；`worker_info.num_workers`
- `collate_fn`：将list of sample拼接成大tensor
  - 每个worker进程均会调用`collate_fn` 
  - 1个batch仅由1个worker合成
- `pin_memory`：若为True，则数据将始终保存在物理内存中，不会被系统交换到虚拟内存（硬盘）
  - 适合True：使用GPU；内存足够；I/O是瓶颈
  - 适合False：使用CPU；内存受限；小数据集
- `prefetch_factor`：每个worker提前加载多少个batch进入内存，默认为2
  - 若已满足，则worker进程阻塞
- `persistent_workers`：每个epoch结束后，是否销毁并重建worker
...
- 使用DataParallel包装model, optimizer
  ...
  - 网络和数据分别需要使用`net.cuda()`和`data.cuda()`
    
    - 不需要指定具体的GPU id
    - `data.cuda()`流程：内存 => GPU0 => GPUx
    - `data.cpu()`流程：GPUx => GPU0 => 内存（主进程内存，不需要做进程间通信）
  
  - 模型保存：`torch.save(net.module.state_dict(), PATH)`
  
  - `dataloader`的`num_workers`，`batch_size` 均为全局数值，为所有GPU之和
...
- DP本质上是多线程，即使推理场景下效率也比DDP低
  
  - 主进程负责所有卡的切分和CPU<=>GPU 互传数据
