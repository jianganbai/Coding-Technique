...
- 创建子进程
  ...
  - spawn: 仅复制主进程必要数据给子进程，子进程重新创建资源
    ...
    - 与CUDA相关，必须使用spawn
    - spawn模式，启动子进程时动态加载。若先执行再修改，则会报错
...
- 创建进程的最基本方法：适合进程数较少
  ...
  - Process对象内置函数
    ...
    - `p.start()`：启动进程；`p.terminate()`：终止进程；
      - `p.terminate()`前使用`mp.Event`告知子进程结束，释放资源，若超过timeout再强制终止
    - `p.is_alive()`：判断进程是否在运行；`p.join(timeout)`：主进程等待子进程p执行完毕
    - `p.daemon=True`：将子进程设置为父进程的守护进程，若父进程意外退出，子进程也会自动释放。需要在`p.start()`前设置
    ...
    - ```shell
      ...
      for a in range(20):
          pool.apply_async(
              ...
              error_callback=lambda error: print(error)  # 子进程报错默认不显示在命令行，需要手动打印报错
          )
      ```
...
- 队列`Queue`：主进程/子进程都能访问到
  ...
  - `mp.Queue`不需要显式关闭，代码结束时会自行释放。若实在需要，可通过`queue.close()`关闭

- 只有`mp.Process`可使用`mp.Queue`，`mp.Pool`应使用`Manager.Queue`
  
  - `Manager.Queue`用法与`mp.Queue`一致，需要先实例化manager，再定义Queue
  
  - `Manager.Queue`需要在任务完成后调用`queue.task_done()`
  
  - `Manager.Queue`需要显式关闭，通过`queue.join()`等队列中的任务都完成后再关闭
...
- `mp.Value(typecode, initial_value, lock=True)`
  ...
  - `typecode`：只能为整数和小数，`'i', 'f', 'd'`
  
  - 带互斥锁，但需要手动调用
    
    - ```python
      with val.get_lock():  # 加锁保护
          val.value += 1
      ```
    
    - 如果仅需知大致数值，则读取可以不用加锁

- `mp.Value`与`manager.Value`
  
  - 功能：二者基本一致。`mp.Value`仅支持整数和小数，`manager.Value`支持任意python 对象
  
  - 机制：`mp.Value`基于shared_memory，`manager.Value`基于进程通信。`mp.Value`效率更高
  
  - 调用：二者都带锁，但`mp.Value`需要手动加锁，`manager.Value`在访问值的时候自动加锁
...
- 取值为True / False 的同步原语
  ...
  - `event.wait(timeout=None)`：阻塞当前进程，直到Event内部标志为True
...
- `threading.Thread(target, name, args, kwargs, daemon)`
  ...
  - `name`：线程名称，可通过`thread.name`获取；`daemon`：是否为守护线程

- 子线程返回值
  
  - 无法像`mp.Pool`通过`proc.get()`拿到返回值
  
  - 法1：主线程创建1个queue，子线程结束前将返回值放入
  
  - 法2：共享可变变量（例如dict）
    
    - 不可变变量无法传引用，例：int, float, str
...
- 线程间同步
  ...
  - ```python
    event = threading.Event()  # 没有直接中断线程的命令，需要给线程发送信号，让线程自行退出
    event.set()  # 设定event为True
    event.wait()  # 阻塞直到event为True
    ```
  
  - ```python
    import queue
    task_queue = queue.Queue()  # 可直接使用标准的Queue。线程安全，不需要加锁
    task_queue.task_done()  # 消费者每完成一个任务，就调用这个函数标明完成。但queue没有将计数器暴露出来，需要自行统计个数
    task_queue.empty()
    task_queue.qsize()
    task_queue.join()  # 阻塞至队列为空
    ```
    
    - `queue.Queue`线程安全。无论元素是什么，有多大，`.get()`和`.put()`都是原子操作
      - `queue.join()`仅用于阻塞；可自动删除队列，若手动删除调用`del queue`
    - 跨进程：向queue中放数据需要序列化，拿数据触发深拷贝
    - 进程内：不会触发序列化和深拷贝，传递的是引用

- 注意事项
  
  - `logging.info` 是线程安全的，但不是进程安全的
